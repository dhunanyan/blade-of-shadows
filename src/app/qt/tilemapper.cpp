...
bool TileMapper::loadFromJsonResource(const QString& levelResourcePath)
{
    ...
    if (levelWidth_ <= 0 || levelHeight_ <= 0 || tileSizePx_ <= 0 || tilesetPath.isEmpty())
    {
        return false;
    }
    ...
    if (tilesetCols <= 0 || tilesetRows <= 0)
    {
        return false;
    }
    ...
    for (const QJsonValue& value : tilesArray)
    {
        ...
        if (x < 0 || y < 0 || srcX < 0 || srcY < 0)
        {
            continue;
        }
        if (x >= levelWidth_ || y >= levelHeight_)
        {
            continue;
        }
        if (srcX >= tilesetCols || srcY >= tilesetRows)
        {
            continue;
        }
